...
can include line breaks.` // Same string type.
	...
	a5 := [...]int{4, 4, 3, 1, 5, 10, 100} // An array initialized with a fixed size of five
	...
	file.Close()
	// _, err := fmt.Fprint(file, "The file is already closed")
	// if err != nil {
	// 	fmt.Println(err)
	// 	os.Exit(1)
	// }
...
func learnMemory() (p, q *int) {
	...
	return &s[2], &r     // & takes the address of an object.
}
...
func learnConcurrency() {
	...
	case i := <-c: // The value received can be assigned to a variable,
		fmt.Printf("it's za %T", i)
	case <-cs: // or the value received can be discarded.
		fmt.Println("it's a stringz")
